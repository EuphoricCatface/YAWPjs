...
class inputManager {
    ...
    static on(event: string, callback: CallableFunction) {
        if (!inputManager.events[event]) { inputManager.events[event] = []; }
    ...
    static selection_clear() {
        ...
        console.assert(inputManager.selected_tiles.length == 0 &&
            ...
            inputManager.word_construct.length == 0,
            "selection_clear: internal selection did not clear!");
    }
    static nextTile(element: HTMLElement, tile: Tile) {
        if (Tile.POINTER_DEBUG) console.log("nextTile start");
        if (element.nodeName == "#text" || element.classList.contains("tileScore")) {
            if (Tile.POINTER_DEBUG) console.log("nextTile: element is likely a child of a Tile - using its parent.");
        ...
        console.assert(
            inputManager.selected_tiles.length == inputManager.selected_elements.length,
            "selected tiles and elements length mismatch"
        );
        console.assert(
            inputManager.selected_elements.length ==
            inputManager.word_construct.length - [...inputManager.word_construct.matchAll(/Qu/gi)].length,
            "selected tiles and word length mismatch"
        );

        do { // do ... while as goto replacement
            if (inputManager.selected_elements.length == 0) {
                if (Tile.POINTER_DEBUG) console.log("nextTile cond 1");
                // start of selection;
                // adding unconditionally
                continue; // goto ADD;
            }
            if (inputManager.selected_elements.at(-1) === element) {
                if (Tile.POINTER_DEBUG) console.log("nextTile cond 2");
            ...
            if (!tile.isNeighbor(inputManager.selected_tiles.at(-1))){
                if (Tile.POINTER_DEBUG) console.log("nextTile cond 3");
            ...
            if (inputManager.selected_elements.length >= 2 &&
                ...
                if (Tile.POINTER_DEBUG) console.log("nextTile cond 4");
            ...
            if (inputManager.selected_elements.find(e => e === element)){
                if (Tile.POINTER_DEBUG) console.log("nextTile cond 5");
                // element is already in the list;
                // ignoring
                return false;
            }
        } while (false); // ADD:
        if (Tile.POINTER_DEBUG) console.log("nextTile cond 6");
    ...
    static sendInput() {
        console.assert(Boolean(inputManager.selected_tiles.length) &&
            ...
            Boolean(inputManager.word_construct.length),
